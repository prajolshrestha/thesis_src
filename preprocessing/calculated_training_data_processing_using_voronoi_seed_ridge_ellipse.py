...
parser.add_argument('--output_folder', type=str, default='./AOSLO_dataset_using_voronoi_seed_ellipse', help='path to output folder')
...
def main():
    ...
    for image_id in tqdm(image_ids):
        ...
        closest_points = []
        farthest_points=[]
        for seed_idx, region_index in enumerate(vor.point_region):
            region = vor.regions[region_index]
            if not -1 in region and len(region) > 0:
                temp = [list(map(int, vor.vertices[i])) for i in region]
                
                # Compute center
                center = cone_center_coords[seed_idx]
                ...
                # Compute radius as the distance fromcentroid to the nearest ridge
                distances = np.linalg.norm(np.array(temp) - center, axis = 1)
                distance_to_ridge = np.linalg.norm(ridge_points - center, axis = 1)
                min_distance_index = np.argmin(distance_to_ridge)
                max_distance_index = np.argmax(distances)
                closest_vertex = np.array(ridge_points[min_distance_index])
