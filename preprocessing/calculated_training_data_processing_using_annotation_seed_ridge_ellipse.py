...
parser.add_argument('--output_folder', type=str, default='./AOSLO_dataset_using_annotation_seed_ridge_ellipse', help='path to output folder')
...
def main():
    ...
    for image_id in tqdm(image_ids):
        ...
        # Compute closest point of a polygon
        closest_points = []
        farthest_points = []
        for center, poly_vertices in zip(cone_center_coords, poly_coords):
            poly_vertices = poly_vertices[0]
            midpoints = []
            for i in range(len(poly_vertices) - 1):
                midpoint = (poly_vertices[i] + poly_vertices[i + 1]) / 2
                midpoints.append(midpoint)

            distances = np.linalg.norm(np.array(poly_vertices) - center, axis = 1)
            distance_to_ridge = np.linalg.norm(np.array(midpoints) - center, axis = 1)
            min_distance_index = np.argmin(distance_to_ridge)
            max_distance_index = np.argmax(distances)
            closest_vertex = np.array(midpoints[min_distance_index])
            farthest_vertex = np.array(poly_vertices[max_distance_index])
        ...
        for center, closest_point, farthest_point in zip(cone_center_coords, closest_points, farthest_points):
            minor = int(np.linalg.norm(center - closest_point))
            major = int(np.linalg.norm(center - farthest_point))
            axes = (major, minor)
            center = tuple(map(int, center))

            direction_vector = farthest_point - center
            angle = np.rad2deg(np.arctan2(direction_vector[1], direction_vector[0])) # Can we use PCA to find orientation?
            start_angle = 0
            end_angle = 360
