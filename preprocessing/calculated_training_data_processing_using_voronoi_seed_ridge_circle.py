...
parser.add_argument('--image_name', type=str, default='4571_left_retina_Confocal_RPE_IS_2023_09_09', help='AOSLO image name')
parser.add_argument('--output_folder', type=str, default='./AOSLO_dataset_using_voronoi_seed_ridge_circle', help='path to output folder')
...
def main():
    ...
    for image_id in tqdm(image_ids):
        ...
        for point_idx, region_index in enumerate(vor.point_region):
            ...
            if not -1 in region and len(region) > 0: # Use bounded regions only!
            #if len(region) > 0: # Use all regions!
                temp = [list(map(int, vor.vertices[i])) for i in region]
                
                # Compute center
                center = np.array(cone_center_coords[point_idx])
            
                # Compute distance to ridge points
                ridge_points = []
                for simplex in vor.ridge_vertices:
                    if simplex[0] in region and simplex[1] in region: # Look current voronoi region!
                ...
                distances = np.linalg.norm(ridge_points - center, axis = 1)
                min_distance_index = np.argmin(distances)
                closest_ridge_vertex = np.array(ridge_points[min_distance_index])

                if np.any(closest_ridge_vertex < 0): # How to remove polygon regions with infinite vertex?
                    continue

                centers.append(center)
                circumference_points.append(closest_ridge_vertex)
