...
parser.add_argument('--output_folder', type=str, default='./AOSLO_dataset_using_voronoi_seed_circle', help='path to output folder')
...
def main():
    ...
    for image_id in tqdm(image_ids):
        ...
        for seed_index, region_index in enumerate(vor.point_region):
            region = vor.regions[region_index]
            if not -1 in region and len(region) > 0: # Use bounded regions
                temp = [list(map(int, vor.vertices[i])) for i in region]
                
                # Compute center
                center = cone_center_coords[seed_index]
                ...
                if np.any(closest_vertex < 0): # How to remove polygon regions with infinite vertex?
                    continue
        ...
        for center, circumference_point in zip(centers, circumference_points):
            radius = int(np.linalg.norm(center - circumference_point) - 0.5)  # substract some value to reduce overlap
            center = tuple(map(int, center))
